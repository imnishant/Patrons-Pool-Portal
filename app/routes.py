...
def update_transaction():
    if request.method == 'POST':
        ...
        return render_template('sponsor.html', posts=posts, title="Home")
...
def add_post():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(extension):
            ...
            return render_template("home.html", posts = posts, profile=res, msg='Allowed file types are mp4, mp3, png, jpg, jpeg, gif', title="Home")
    return render_template("home.html", posts = posts, profile=res, msg='Added Successfully Bro! :-D', title="Home")
...
def home():
    if session['isSponsor'] == 1:
        ...
        res = get_profile(session['username'])
        posts = get_posts(session['username'])
        return render_template('home.html', posts=posts, profile=res, title="Home")
...
def delete_post():
    if request.method == 'POST':
        ...
        if not res:
            return render_template('access_denied.html', error_msg="Error Occurred while fetching Profile Details", title="Error")
        return render_template('home.html', posts=posts, profile=res, msg="Post Successfully deleted!", title="Home")
    ...
    if request.method == 'POST':
        ...
        if bool(target_post):
            ...
            if not earlier and new_sponser_bid_price > base_price:
                ...
                posts = get_sponser_timeline()
                return render_template("sponsor.html", posts=posts,msg='Please enter amount greater than the current bid amount!', title="Home")

            else:
                # display time window is over you cannot bid anymore
                posts = get_sponser_timeline()
                return render_template("sponsor.html", posts=posts,
                                       msg='You cannot bid anymore because Bidding Time is Over', title="Home")

            posts = get_sponser_timeline()
            return render_template("sponsor.html", posts=posts, msg='Your Bid Placed Successfully Bro! ATB! :)', title="Home")
        else:
            return render_template('access_denied.html', error_msg="File does not exist in mongodb database", title="Error")
    return render_template('access_denied.html', error_msg="Delete Post Method is not POST", title="Error")
...
def search():
    ...
    if session['isSponser'] == 0:
        if not result:
            return render_template('search-home.html', error_msg="Oops! No Search Result Found for Query: ", found="no", username="None", query=query, title="Search")
        return render_template('search-home.html', result=result, found="yes", msg='Search result for Query: ', query=query, title="Search")

    if session['isSponser'] == 1:
        posts = get_sponser_timeline()
        if not result:
            return render_template('search-sponser.html', error_msg="Oops! No Search Result Found for Query: ", found="no", username="None", query=query, posts=posts, title="Search")
        return render_template('search-sponser.html', result=result, found="yes", msg='Search result for Query: ', query=query, posts=posts, title="Search")
    return
